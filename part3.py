...
def chisquare(A1,A2,alpha ):
	w1w2 = A1 + " " +A2
	N=float(len(bigram_list))
	O11=float(s.count(w1w2))
	O21=float(s.count(A1)-O11)
	O12=float(s.count(A2)-O11)
	O22=float(len(bigrams)-O12 -O21 + O11)
	# print O11
	# print O12
	# print O21
	# print O22 
	if(( (O11+O12)*(O11+O21)*(O12+O22)*(O21+O22) ) == 0):
		return 0
...
s = ''.join(map(str, sentences))
alpha='0.050'
print "starting chisquare test"
num_collocs = 0
bar = progressbar.ProgressBar(maxval=len(bigram_list),widgets=[progressbar.Bar('=', '[', ']'), ' ', progressbar.Percentage()])
bar.start()
for idx, val in enumerate(bigram_list):
	words = bigram_list[idx].split(" ")
	num_collocs += chisquare(words[0],words[1],alpha)
	bar.update(idx+1)
bar.finish()
print "Number of bigrams =",len(bigram_list)
print "Number of Collocations =",num_collocs